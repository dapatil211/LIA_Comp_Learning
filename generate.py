...
from shapeworld.datasets.agreement.logical import LogicalDataset
...
DATASET = LogicalDataset(entity_counts=[4, 5],
            collision_tolerance=0.0,
            boundary_tolerance=0.0,
            train_correct_ratio=1.0,
            captioners=['existential'],
            connectives=['conjunction'])
random = np.random.RandomState(0)

all_captions = {}
while len(all_captions) < N_CAPTIONS:
    if len(all_captions) % 100 == 0:
        print("%d / %d captions" % (len(all_captions), N_CAPTIONS))

    
    while not DATASET.world_generator.initialize(mode="train"):
        pass
    while not DATASET.world_captioner.initialize(mode="train", correct=True):
        pass
    while True:
        world = DATASET.world_generator()
        if world is None:
            continue
        caption = DATASET.world_captioner(world=world)
        if caption is None:
            continue
        break
...
def generate(name, captions, n_examples):
    ...
    #for _ in range(n_examples * 5):
        DATASET.world_generator.initialize(mode="train")
        world = DATASET.world_generator()
        if world is None:
            continue
        for key in captions:
            ...
            agree = caption.agreement(predication=predication, world=world)
