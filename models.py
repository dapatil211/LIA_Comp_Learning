...
def examples_encoder(examples):
    ...
    examples = tf.map_fn(lambda x: tf.map_fn(lambda y: tf.map_fn(lambda z: tf.concat([y, z], 0), x), x), examples)
...
"""
def baseline_model_1(desc, lens, examples, inputs, labels):
    embeddings = elmo(desc)
    rnn_cell = tf.nn.rnn_cell.BasicRNNCell(128)
    initial_state = rnn_cell.zero_state(BATCH_SIZE, dtype=tf.float32)
    outputs, state = tf.nn.dynamic_rnn(rnn_cell, embeddings,
    ...
    x = tf.layers.dense(concept_rep, 512, activation=tf.nn.relu)
...
def baseline_model_2(desc, lens, examples, inputs, labels):
    ...
    concept_rep = tf.layers.dense(concept_rep, 128 * 2 * 2, activation=tf.nn.relu)
    dist = tfd.MultivariateNormalDiag(concept_rep[:, :128 * 2], concept_rep[:, 128 * 2:])
    # Conjunction module
    # concept_rep = tf.layers.dense(concept_rep, 512, activation=tf.nn.relu)
...
def model_1(desc, lens, examples, inputs, labels):
    ...
    rnn_cell = tf.nn.rnn_cell.BasicRNNCell(128)
    initial_state = rnn_cell.zero_state(BATCH_SIZE, dtype=tf.float32)
    outputs_0, state_0 = tf.nn.dynamic_rnn(rnn_cell, embeddings_0,
                                sequence_length=lens_0,
                                initial_state=initial_state,
                                dtype=tf.float32)
    
    outputs_1, state_1 = tf.nn.dynamic_rnn(rnn_cell, embeddings_1,
...
def model_2(desc, lens, examples, inputs, labels):
    ...
    text_concept_rep = tf.layers.dense(text_concept_rep, 512, activation=tf.nn.relu)
    text_concept_rep = tf.layers.dense(text_concept_rep, 128 * 2 * 2, activation=tf.nn.relu)
    text_dist = tfd.MultivariateNormalDiag(text_concept_rep[:, :128 * 2], text_concept_rep[:, 128 * 2:])

    example_code = examples_encoder(examples)
    image_concept_rep = tf.layers.dense(example_code, 512, activation=tf.nn.relu)
    image_concept_rep = tf.layers.dense(example_code, 128 * 2 * 2, activation=tf.nn.relu)
    image_dist = tfd.MultivariateNormalDiag(image_concept_rep[:, :128 * 2], image_concept_rep[:, 128 * 2:])
